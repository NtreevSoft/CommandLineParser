...
"%NSDEPLOY%\version-builder.exe" %version% --output-path "%cur_path%\Ntreev.Library.Commands.Sharing.AssemblyInfo\AssemblyInfo.cs" --configuration %configuration% --company %company% --copyright %copyright%
set result=%errorlevel% 
if not %result%==0 goto fail
